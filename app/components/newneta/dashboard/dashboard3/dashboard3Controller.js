angular
    ...
    function($scope,FileQueueFactory,$log,$filter,show_config_ui_month,show_config_ui_year,show_config_ui_file_id,
             $rootScope,$compile,variables,ReconstructMenu) {
        ...
        var listDataRep = $scope.listDataRep = [
            'input','collect','waiting','onProcess','finished','error'
        ];
        ...
        $scope.showDetail = function(data,action) {
            //$scope.selectedData = data;
            $scope.action_param = action;
            $scope.action_show = 'LIST DATA '+action.toUpperCase();
            var dto_onProcessList = data.onProcessList;
            var dto_finishedList = data.finishedList;
            var dto_errorList = data.errorList;
            var dto_waitingList = data.waitingList;

            var allData = [];
            allData.push.apply(allData,dto_onProcessList);
            allData.push.apply(allData,dto_finishedList);
            allData.push.apply(allData,dto_errorList);
            allData.push.apply(allData,dto_waitingList);

            $scope.selectedDataList = {
                input:[],collect:[],waiting:[],onProcess:[],finished:[],error:[]
            };

            $scope.selectedDataList.input = allData;
            $scope.selectedDataList.collect = allData;
            $scope.selectedDataList.waiting = dto_waitingList;
            $scope.selectedDataList.onProcess = dto_onProcessList;
            $scope.selectedDataList.finished = dto_finishedList;
            $scope.selectedDataList.error = dto_errorList;
        ...
        $scope.submit = function(year,month,fileId) {
            var temp = {
                "yearMonth":year+month,
                "fileId":fileId
            };
            listDays = $scope.listDays = getDaysInMonth(parseInt(month)-1,parseInt(year));
            if (year == null || month == null || fileId == null) {
                ...
                $scope.notification.mainTable = true;
                FileQueueFactory.FileQueueReport(temp)
                    .then(function(data){
                        ////$log.info('success');
                        angular.forEach(listDays, function(days) {
                            try {
                                test_data[days]=$filter('filter')(data, {day:days})[0].report;
                            }
                            catch(err) {
        ...
        function InitData() {
            ...
            listDays = $scope.listDays = getDaysInMonth(_month,_year);
            ////$log.info('temp : '+JSON.stringify(temp));

            FileQueueFactory.FileQueueReport(temp).then(function(data){
                    angular.forEach(listDays, function(days) {
                        try {
                            test_data[days]=$filter('filter')(data, {day:days})[0].report;
                        }
        ...
        var file_queue_report = {
            method: 'POST',
            url: ServerConfig.production + '/neta/wifiid/search/file_queue_report',
            headers: {},
            params:{}
        };

        service.FileQueueReport= function(data) {
            var deffered = $q.defer();

            file_queue_report.data = data;

            $http(file_queue_report).
