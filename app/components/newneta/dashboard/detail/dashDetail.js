angular
    .module('altairApp')
    .controller('dashDetailCtrl',function ($rootScope,$scope,$interval,$timeout,$compile,$log,$stateParams,
                                           ActionDetail,ReconstructMenu) {
        //ReconstructMenu.Init();
        ReconstructMenu.Init();
        //--------------------------------------------------------------------------------------------------------------
        var action = $scope.action = $stateParams.action;
        $scope.show = {
            file_queue_detail:true,
            file_queue_detail_info:false
        };
        //--------------------------------------------------------------------------------------------------------------
        // request detail data
        var success_request_detail = function(data) {
            ////$log.info('Success Get Detail Data');
            ////$log.info('data : '+data.value);
            /*$scope.show.file_queue_detail_info = true;*/
            if (action == 'waiting') {
                $scope.dataListWaiting = data.dataList;
            } else if (action == 'finished') {
                $scope.dataListFinishing = data.dataList;
                ////$log.info('$scope.dataListFinishing : '+$scope.dataListFinishing.length);
            } else if (action == 'processing') {
                $scope.dataListProcessing = data.dataList;
            } else if (action == 'error') {
                //$log.info('data : '+JSON.stringify(data));
                $scope.dataListError = data.dataList;
                //$log.info('$scope.dataListError : '+JSON.stringify($scope.dataListError));
            }

        };
        var error_request_detail = function(data) {
            $log.error('Error In dashDetail.js, message : '+data);
        };
        $timeout(function(){
            ActionDetail.getDetail($scope.action).then(success_request_detail,error_request_detail);
        },500);
        //--------------------------------------------------------------------------------------------------------------
        // action button
        $scope.submit_file_queue_detail_info = function(filename) {
            ////$log.info('filename : '+filename);
            $scope.show.file_queue_detail = false;
            $scope.show.file_queue_detail_info = true;
            ////$log.info('$scope.show.file_queue_detail_info : '+$scope.show.file_queue_detail_info);

            function success_file_queue_detail_info_req(data) {
                ////$log.info('Success, data : '+JSON.stringify(data));
                $scope.file_queue_detail_info = data;
                ////$log.info('$scope.file_queue_detail_info.dataCollectionReport : '+JSON.stringify($scope.file_queue_detail_info.dataCollectionReport));
            }
            function error_file_queue_detail_info_req(data) {
                //$log.info('Error : '+data);
            }
            ActionDetail.FileQueueDetailInfo(filename).then(success_file_queue_detail_info_req,error_file_queue_detail_info_req);


        };
        $scope.exit_file_queue_detail_info = function() {
            ////$log.info('exit');
            $scope.show.file_queue_detail = true;
            $scope.show.file_queue_detail_info = false;
            ////$log.info('$scope.show.file_queue_detail_info : '+$scope.show.file_queue_detail_info);
        };
        //--------------------------------------------------------------------------------------------------------------
        // config button
    ...
    .factory('ActionDetail', function($timeout,$log,$http,ServerConfig,$q){
        var service = {};

        var action_detail = {
            method: 'GET',
            url: ServerConfig.production + '/neta/wifiid/dashboard/file_queue/detail',
            params:{action:''},
            headers: {}
        };


        service.getDetail = function(param) {
            ////$log.info('param : '+param);
            var deffered = $q.defer();
            action_detail.params.action=param;
            $http(action_detail).
        ...
        var file_queue_detail_info = {
            method: 'GET',
            url: ServerConfig.production + '/neta/wifiid/dashboard/file_queue_detail_info',
            headers: {},
            params:{filename:''}
        };

        service.FileQueueDetailInfo = function(filename) {
            var deffered = $q.defer();
            file_queue_detail_info.params.filename = filename;
            $http(file_queue_detail_info).
                success(function(data,status,headers) {
                //$log.info('====================data : '+JSON.stringify(data));
                    deffered.resolve(data);
                }).error(function(data,status){
